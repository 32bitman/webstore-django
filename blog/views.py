...
class BlogCreate(SuccessMessageMixin, CreateView, CategoryListMixin):
    ...
    success_message = "Артыкул паспяхова дададены"
    fields = "__all__"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.blog.user == request.user or request.user.is_superuser:
            ...
            if self.form.is_valid:
                self.form.save()
                messages.add_message(request, messages.SUCCESS, "Артыкул паспяхова зменены")
                redirect_url = reverse("blog_index") + "?page=" + self.request.GET["page"]
                try:
                    redirect_url += "&search=" + self.request.GET["search"]
                except KeyError:
                    pass
                try:
                    redirect_url += "&tag=" + self.request.GET["tag"]
                except KeyError:
                    pass
...
class BlogDelete(PageNumberView, TemplateView, SearchMixin, PageNumberMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.blog.user == request.user or request.user.is_superuser:
            self.blog.delete()
            messages.add_message(request, messages.SUCCESS, "Артыкул паспяхова выдалены")
            redirect_url = reverse("blog_index") + "?page=" + self.request.GET["page"]
            try:
                redirect_url += "&search=" + self.request.GET["search"]
            except KeyError:
