...
GoodImagesFormset = inlineformset_factory(Good, GoodImage, can_order=True, fields=["good", "image"])
...
class GoodCreate(PageNumberView, TemplateView, SortMixin, PageNumberMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.formset.is_valid():
            self.formset.save()
            messages.add_message(request, messages.SUCCESS, "Тавар паспяхова дададены")
...
class GoodUpdate(PageNumberView, TemplateView, SortMixin, PageNumberMixin):
    ...
    def get_context_data(self, **kwargs):
        context = super(GoodUpdate, self).det_context_data(**kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if self.formset.is_valid():
                self.formset.save()
                messages.add_message(request, messages.SUCCESS, "Тавар паспяхова зменены")
...
class GoodDelete(PageNumberView, DeleteView, SortMixin, PageNumberMixin):
    ...
    def post(self, request, *args, **kwargs):
        self.success_url = reverse("goods_index", kwargs={"pk": Good.objects.get(pk=kwargs["pk"]).category.pk}) + \
                           "?page=" + self.request.GET["sort"] + "&order=" + self.request.GET["order"]
        messages.add_message(request, messages.SUCCESS, "Тавар паспяхова выдалены")
...
class GoodsListFeed(Feed):
    ...
    def subtitle(self):
        return "Новыя паступленні"

    def link(self):
        return reverse("GoodListView")
